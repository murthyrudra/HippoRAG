...
from demo_local import api_key
import httpx
import openai
...
def cache_response(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        gen_params = (
            getattr(self, "llm_config", {}).generate_params
...
class CacheOpenAI(BaseLLM):
        ...
        **kwargs,
        ...
        if self.global_config.azure_endpoint is None:
            self.openai_client = OpenAI(
                ...
                default_headers={
                    "RITS_API_KEY": self.global_config.extra_headers["RITS_API_KEY"],
                },
            )
        else:
    ...
    def infer(
        ...
        if "gpt" not in params["model"] or version.parse(
            openai.__version__
        ) < version.parse(
            "1.45.0"

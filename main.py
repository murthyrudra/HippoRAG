...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="2wikimultihopqa", help="Dataset name"
    )
    parser.add_argument(
        "--llm_base_url",
        type=str,
        default="https://inference-3scale-apicast-production.apps.rits.fmaas.res.ibm.com/gpt-oss-120b/v1",
        help="LLM base URL",
    )
    parser.add_argument(
        "--llm_name", type=str, default="openai/gpt-oss-120b", help="LLM name"
    )
    parser.add_argument(
        "--embedding_name",
        type=str,
    ...
    corpus_path = f"reproduce/{dataset_name}_corpus.json"
    ...
    samples = json.load(open(f"reproduce/{dataset_name}.json", "r"))
